...
# ─────────────────────────────────────────────
# /SETUP — Step 1: ask for main channel
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# Text handler — interactive setup flow
# ─────────────────────────────────────────────
...
async def handle_setup_input(client, message: Message):
    ...
    if state["step"] == "awaiting_main_channel":
        try:
            ...
            # Verify bot is admin in the main channel
    ...
    if state["step"] == "awaiting_duplicate":
        # User typed "done" → finalize
        if text.lower() == "done":
            if not state["duplicates"]:
                await message.reply_text(
                    "❌ You didn't add any duplicate channels. Use /setup again.",
                    parse_mode=ParseMode.MARKDOWN
                )
                del setup_states[user_id]
                return

            # Check if this main channel already has a mapping
            ...
            if existing_mapping:
                # Merge duplicates into existing mapping
                for dup in state["duplicates"]:
                    if dup not in existing_mapping["duplicates"]:
                        existing_mapping["duplicates"].append(dup)
                save_config(config)
        ...
        # Otherwise, try to resolve the duplicate channel
        try:
            if text.startswith("@"):
                chat = await client.get_chat(text)
            elif text.startswith("-100") or text.lstrip("-").isdigit():
...
if __name__ == "__main__":
    try:
        ...
        logging.exception("Fatal Error")def save_config(config):
